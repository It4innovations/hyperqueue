...
use crate::common::env::{
    HQ_ACCESS_KEY, HQ_CPUS, HQ_ENTRY, HQ_ERROR_FILENAME, HQ_HOST_FILE, HQ_INSTANCE_ID, HQ_JOB_ID,
...
pub(super) fn build_program_task(
    ...
    let (program, task_id, instance_id, task_dir): (
        ...
        let key = build_ctx.access_key();
        program.env.insert(HQ_ACCESS_KEY.into(), key);
        program.env.insert(
            "HQ".into(),
            std::env::current_exe()
                .unwrap()
