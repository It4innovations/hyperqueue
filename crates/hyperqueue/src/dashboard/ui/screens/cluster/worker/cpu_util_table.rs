    ...
    fn get_visible_indices(
        ...
        match self {
            ...
            CpuViewMode::WorkerManaged => match cpu_scope {
                CpuScope::Node => (0..total_cpus)
                    .map(|idx| ResourceIndex::new(idx as u32))
                    .collect(),
                CpuScope::Subset => cpu_state.managed_cpus.clone(),
            },
...
impl CpuUtilTable {
    ...
    pub fn draw(&mut self, rect: Rect, frame: &mut DashboardFrame) {
        if let (Some(util), Some(cpu_state)) = (&self.utilization, &self.cpu_state) {
            ...
            let cpu_scope = self.get_current_scope();

            let visible_indices =
                self.cpu_view_mode
                    .get_visible_indices(&cpu_scope, util.cpu.len(), cpu_state);
            ...
            let (which_util, num_cpus, avg_cpu) =
                create_title_info(&util.cpu, cpu_state, &self.cpu_view_mode, &cpu_scope);
    ...
    let filtered_utils: Vec<f64> = cpu_util_list
        ...
        .filter(|(idx, _util)| {
            ...
            match util_render_mode {
                ...
                CpuViewMode::WorkerAssigned => cpu_state.assigned_cpus.contains(&res_idx),
                CpuViewMode::WorkerManaged => match cpu_scope {
                    CpuScope::Node => true,
                    CpuScope::Subset => cpu_state.managed_cpus.contains(&res_idx),
                },
            }
        })
        .map(|(_, &util)| util)
