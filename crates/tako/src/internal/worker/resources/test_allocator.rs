...
fn simple_allocator(free: &[ResourceUnits], running: &[&[ResourceUnits]]) -> ResourceAllocator {
...
fn simple_alloc(allocator: &mut ResourceAllocator, counts: &[ResourceUnits], expect_pass: bool) {
    ...
    let al = allocator.try_allocate(&builder.finish());
    if expect_pass {
        al.unwrap();
    } else {
        assert!(al.is_none());
...
fn test_allocator_single_socket() {
    let mut allocator = simple_allocator(&[4], &[]);
...
fn test_pool_generic_resources() {
    ...
    let rq = cpus_compact(1).add(4, 2).finish();
    assert!(allocator.try_allocate(&rq).is_none());
...
fn test_allocator_fractions_compactness() {
    ...
    allocator.release_allocation(al4);
    let al5 = allocator.try_allocate(&rq3).unwrap();
    allocator.release_allocation(al3);
...
fn test_allocator_groups_and_fractions() {
    ...
    let rq3 = ResBuilder::default()
        ...
        .finish();
    let al3 = allocator.try_allocate(&rq3).unwrap();
    ...
    let rq4 = ResBuilder::default()
        ...
        .finish();
    let al4 = allocator.try_allocate(&rq4).unwrap();
    ...
    let rq6 = ResBuilder::default()
        ...
        .finish();
    let al6 = allocator.try_allocate(&rq6).unwrap();
...
fn test_coupling1() {
    for i in 0..=2 {
        ...
        let rq2 = cpus_compact(2).add(2, 2).finish();
        let al3 = allocator.try_allocate(&rq2).unwrap();
...
fn test_coupling2() {
    ...
    let mut allocator = test_allocator(&descriptor);
    let rq1 = cpus_compact(4).add(1, 3).finish();
    let al1 = allocator.try_allocate(&rq1).unwrap();
    allocator.validate();
    let s1 = allocator.get_sockets(&al1, 0);
