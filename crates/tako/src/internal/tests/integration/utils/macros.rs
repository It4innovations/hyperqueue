...
macro_rules! wait_for_msg {
    ($handle: expr, $matcher:pat $(=> $result:expr)?) => {
        $handle.recv_msg(|msg| match msg {
            $matcher => $crate::internal::tests::integration::utils::server::MessageFilter::Expected(($($result),*)),
            _ => $crate::internal::tests::integration::utils::server::MessageFilter::Unexpected(msg),
        }).await
    };
    ($handle: expr, $matcher:pat if $guard:expr $(=> $result:expr)?) => {
        $handle.recv_msg(|msg| match msg {
            $matcher if $guard => $crate::internal::tests::integration::utils::server::MessageFilter::Expected(($($result),*)),
            _ => $crate::internal::tests::integration::utils::server::MessageFilter::Unexpected(msg),
        }).await
    }
}
...
pub(in crate::internal::tests::integration) use wait_for_msg;
