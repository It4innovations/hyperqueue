...
def test_job_submit_program_not_found(hq_env: HqEnv):
    ...
    table = hq_env.command(["task", "list", "1", "-v"], as_table=True)
    assert (
        'Cannot execute "foo --bar --baz=5": No such file or directory (os error 2)\n'
...
def test_job_submit_program_not_found_file_exists(hq_env: HqEnv):
    ...
    table = hq_env.command(["task", "list", "1", "-v"], as_table=True)
    assert f"""Cannot execute "foo": No such file or directory (os error 2)
...
def test_job_shell_script_fail_not_executable(hq_env: HqEnv):
    ...
    table = hq_env.command(["task", "list", "1", "-v"], as_table=True)
    assert """Cannot execute "./test.sh": Permission denied (os error 13)
...
def test_custom_error_message(hq_env: HqEnv):
    ...
    table = hq_env.command(["task", "list", "1", "-v"], as_table=True)
    assert table.get_column_value("Error")[0] == "Testing message"
    # print(table)
...
def test_zero_custom_error_message(hq_env: HqEnv):
    ...
    table = hq_env.command(["task", "list", "1", "-v"], as_table=True)
    assert table.get_column_value("Error")[0] == "Task created an error file, but it is empty"
    # print(table)
...
"""
    ...
    table.check_row_value("Error", "Program terminated with exit code -1. Received signal 9 (SIGKILL).")
...
def test_fast_retracting(hq_env: HqEnv):
    ...
    hq_env.start_worker(cpus=4, args=["--time-limit=80s", "--retract-check-interval=500ms"])
    wait_for_job_state(hq_env, 1, "RUNNING")
