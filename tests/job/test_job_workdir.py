...
def test_job_workdir_with_complex_working_directories(hq_env: HqEnv):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Job with custom cwd
        hq_env.command(["submit", "--name", "custom-cwd", "--cwd", subdir, "--", "pwd"])

        # Job with relative cwd
        rel_path = os.path.relpath(subdir)
        hq_env.command(["submit", "--name", "relative-cwd", "--cwd", rel_path, "--", "pwd"])
...
def test_job_workdir_json_structure(hq_env: HqEnv):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        hq_env.command(["submit", "--cwd", tmpdir, "--array", "1-2", "--", "echo", "test"])
...
def test_job_workdir_with_stdout_stderr_redirection(hq_env: HqEnv):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        hq_env.command(
            ["submit", "--stdout", stdout_file, "--stderr", stderr_file, "--cwd", tmpdir, "--", "echo", "hello"]
        )
        wait_for_job_state(hq_env, 1, "FINISHED")
...
def test_job_workdir_consistency_with_job_info(hq_env: HqEnv):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        hq_env.command(["submit", "--cwd", tmpdir, "--", "echo", "test"])
        wait_for_job_state(hq_env, 1, "FINISHED")
