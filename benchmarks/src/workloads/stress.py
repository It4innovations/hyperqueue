...
class Stress(Workload):
    ...
    def compute(self, env: Environment, task_count: int, cpu_count: int,
                stress_duration: int) -> WorkloadExecutionResult:
...
class StressHQ(Stress):
    def compute(self, env: Environment, task_count: int, cpu_count: int,
                stress_duration: int) -> WorkloadExecutionResult:
        return measure_hq_tasks(env, [
            "stress",
            "--cpu", str(cpu_count),
            "--timeout", str(stress_duration)
        ], task_count=task_count, cpus_per_task=cpu_count)
