...
use std::fmt::Display;
use cli_table::{Cell, CellStruct, Table, Style, Color};

pub(crate) fn handle_message(message: crate::Result<ToClientMessage>) -> crate::Result<ToClientMessage> {
    match message {
        Ok(msg) => match msg {
...
impl OutputStyle {
    ...
    pub fn apply_on_rows(&self, mut table: Vec<Vec<CellStruct>>) -> Vec<Vec<CellStruct>> {
        if self.plain_style {
            table.into_iter().map(|row| row.into_iter().map(|cell| {
                cell.bold(false).background_color(None).foreground_color(None)
            }).collect()).collect()
        } else {
            table
        }
